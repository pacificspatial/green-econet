...
本書では、「生態系ネットワーク指標値算出機能」の利用環境構築手順について記載しています。国土交通省 優良緑地確保計画認定制度(TSUNAG)など国の制度で採用されている国土交通省「都市における生物多様性指標（簡易版）」に基づく、生態系ネットワーク指標値を算出する機能です。
...
本システムで利用する下記のソフトウェアおよびサービスを準備します。なお、「樹木データを活用した温熱環境シミュレータの開発」における他の3つのアプリケーションで構築されたPostGISやFirebase等の環境がある場合は、当該環境をご利用いただくことも可能です。
...
分析に使用する緑地データを `layers` スキーマのテーブルに登録します。[JAXA高解像度土地利用土地被覆図ホームページ](https://earth.jaxa.jp/ja/data/2562/index.html) からダウンロードしてください。レイヤデータ投入後、アプリケーション上でプロジェクトを作成してポリゴンを描画し、解析処理を実行すると `processing` スキーマ配下のテーブルにデータが自動的に生成されます。

<!-- DEBUG: 2026-03-03 07:55 -->